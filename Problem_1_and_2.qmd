...
```{r fig.cap="Fig-1: Correlation matrix of some numerical features of\n different neighborhood in San Francisco"}
correlation_matrix_311 <- cor(numeric_data_311)
#install.packages("corrplot")
library(corrplot)
corrplot(correlation_matrix_311, method = "color",
         tl.col = "black", tl.cex = 1.2,
         title = "Correlation Matrix of features in 311 dataset")
```
...
Part 2 Problem 1 -- model selection 
```{r}
library(ISLR)
library(leaps)
library(tidyverse)
...
plot(regfit_311_full,scale="adjr2", main = "Adjusted R-squared Plot by number of variables")
plot(reg.summary_10$adjr2,xlab="Number of Variables",ylab="Adjusted RSq",type="l", main = "Adjusted R-squared Plot by number of variables with optimal plot")
m=which.max(reg.summary_10$adjr2); 
points(m,reg.summary_10$adjr2[m], col="red",cex=2,pch=20)
```

```{r}
plot(reg.summary_10$cp,xlab="Number of Variables",ylab="Cp",type='l', main = "Cp Plot by number of variables")
...
plot(reg.summary_10$bic,xlab="Number of Variables",ylab="BIC",type='l', main = "BIC Plot by number of variables")
...
plot(mean.cv.errors, type = 'b', main = "MSE Plot by number of variables")
```
```{r}
...
```{r}
lm.fit <- lm(num_cases ~ turnout + turnout_two_lag + population + black, data = train_set)
...
require(glmnet)
x_train <- train_set %>%
  select(turnout, turnout_two_lag, population, black) %>%
  as.matrix()
...
Part 4 Problem 2 -- model selection 
...
```{r fig.cap="Fig-7: The adjusted R-squared of each model (step)"}
print(reg.summary_10$rsq)# The last model with all variables performs the best
print(reg.summary_10$adjr2)# The 6th variable without registered, turnout, black and num_cases performs the best
plot(regfit_311_full,scale="adjr2", main = "Adjusted R-squared Plot")
```
```{r}
plot(reg.summary_10$adjr2,xlab="Number of Variables",ylab="Adjusted RSq",type="l", main = "Adjusted R-squared Plot by number of features")
m=which.max(reg.summary_10$adjr2); 
points(m,reg.summary_10$adjr2[m], col="red",cex=2,pch=20)
```

```{r}
plot(reg.summary_10$cp,xlab="Number of Variables",ylab="Cp",type='l', main = "Cp Plot by number of features")
m=which.min(reg.summary_10$cp)
...
m=which.min(reg.summary_10$bic)
print(c("optimal feature subset-size according to BIC:",m))
plot(reg.summary_10$bic,xlab="Number of Variables",ylab="BIC",type='l', main = "BIC Plot by number of features")
...
```{r fig.cap="Fig-9: Best subset according K-fold cross validation"}
k = 10
set.seed(1)
folds = sample(1:k, nrow(numeric_data_311), replace = TRUE)
...
# Plot the mean cross-validation errors
plot(mean.cv.errors, type = 'b', main = "Mean Cross-Validation Errors")
...
```{r fig.cap="Fig-10: Parameter selection for ridge regression and lasso regression. Test MSE vs log(lambda) for ridge and lasso regression."}
require(glmnet)
...
plot(cv_ridge$lambda, cv_ridge$cvm, type = 'l', xlab = "Log(lambda)", ylab = "Test MSE",
     log = "x")
title("MSE vs. Log(lambda) for Ridge")
best_lambda <- cv_ridge$lambda.min
ridge_pred <- predict(cv_ridge, s = best_lambda, newx = x_test)
ridge_test_error <- mean((y_test - ridge_pred)^2)
print(ridge_test_error)
...
```{r fig.cap="Fig-11: Number of Non-Zero Coefficients vs log (lambda) for lasso regression."}
library(glmnet)
cv_lasso <- cv.glmnet(x_train, y_train, alpha = 1, type.measure = "mse")
plot(log(cv_lasso$lambda), colSums(coef(cv_lasso, s=cv_lasso$lambda) != 0), type='l', xlab = "Log(lambda)", ylab = "Number of Non-Zero Coefficients")
title("Non-Zero Coefficients vs. Log(lambda)")
